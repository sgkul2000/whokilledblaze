    ...
    <div v-if="userId != null" class="chat__header">
        <vs-navbar class="p-4 flex navbar-custom" color="white" type="flat">
            <div class="relative flex mr-4">
                ...
                <vs-avatar class="m-0 border-2 border-solid border-white" size="40px" :src="userData.photoURL" @click.stop="$emit('showProfileSidebar', userId)" />
...
export default {
  ...
  methods: {
    getUserStatus (isActiveUser) {
      return isActiveUser ? this.$store.state.AppActiveUser.status : this.userData.status
    }
  }
}
